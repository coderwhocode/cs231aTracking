...
void segmentObjectsTemplates(Scene& seed_frame, Scene& sc) {

  vector<TrackedObject>& seed_objects = seed_frame.segmentation_->tracked_objects_; 
  vector<TrackedObject> tracked_objects(seed_objects.size());

  for (size_t i = 0; i < seed_objects.size(); ++i) {
    TrackedObject obj = seed_objects[i];
    for (size_t j = 0; j < obj.indices_.size(); ++j) {
      int index = obj.indices_[j];
      tracked_objects[i].indices_.push_back(index);
    }
  }
...
int main(int argc, char** argv)
{
  ...
  for(size_t i = 1; i < seq.size(); ++i) {
    ...
    segmentObjectsTemplates(seed_frame, sc);
